(function(SOP){
    ...
    SOP.Dependency.changed = function(event, eventArr, handlerObject)
    {
    ...
    SOP.Dependency.registerHandler = function(fieldId, htmlId)
    {   
    	...
    	(function(){
        	YAHOO.util.Event.addListener(htmlId, "change", function(){
        		...
        		YAHOO.Bubbling.fire("SOP.changed", {itemId:fieldId, itemValue:itemValue});
    ...
    SOP.LoadLabel = function(htmlId)
	{
		var componentName = "SOP.LoadLabel";
		var returnObject = SOP.LoadLabel.superclass.constructor.call(this, componentName, htmlId, "SOP.LoadLabel"); 
		
		return returnObject;
	};
	
	YAHOO.extend(SOP.LoadLabel, Alfresco.component.Base,
	{
		options : {
			itemId : "",
			picklistName : "",
		...
		loadLabels : function() {
            ...
            {
               url: Alfresco.constants.PROXY_URI + 'com/sop/picklist/picklist?includeBlankItem=true&name='+this.options.picklistName+'&itemId='+this.options.itemId+'&initialValues='+this.options.initialValue+dependencyQuery+'&level='+this.options.level,
               method: "GET",
		...
		onReady : function() {
			if (this.options.dependsOn.length > 0)
			{
                YAHOO.Bubbling.on("SOP.changed", SOP.Dependency.changed, this );
