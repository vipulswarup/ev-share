...
(function(SOP)
{
	<#if form.mode == "view">
		<#if field.control.params['loadLabel']??>
			new SOP.LoadLabel("${fieldHtmlId}-value").setOptions(
			{
	...
	<#else>
		...
		new SOP.DynamicDropdown("${fieldHtmlId}-entry").setOptions(
		{
			picklistName:"${field.control.params['picklistName']}",
			dependsOn:[
...
(function(SOP)
{
	SOP.Dependency.registerHandler("${field.id}","${fieldHtmlId}-entry");
})(window.SOP = window.SOP || {});
//]]></script>
